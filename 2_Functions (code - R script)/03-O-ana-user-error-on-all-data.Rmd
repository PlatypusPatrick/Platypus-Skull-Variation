---
title: "03-O-ana-user-error-on-all-data"
...
```{r}
data.raw <- as.matrix(X02_O_ana_Landmark_Data)
data <- X02_O_ana_Landmark_Data[, 2:(dim(X02_O_ana_Landmark_Data)[2])]  # removes LM names for geomorph
data <- t(data)
A <- arrayspecs(data, 135, 3)  # convert to 3D array w 405 pts & 3D coords

plot3d(A[,,1], asp=FALSE)
...
Interspersed throughout the data collection process, AEM landmarked 10 specimens twice. Their specimen names included "_ERR", however, not always in the same place in the filename. 
...
```{r}
findMeanSpec(bY.shape)

par <- gridPar(pt.bg="pink", pt.size = 1)

#vw GO THROUGH all three same way as the line below to check for errors
plotRefToTarget(bY.shape[ , , 2], bY.shape[ , , 17], method = "vector", gridPars = par )
plotRefToTarget(bY.shape[ , , outliers[2]], bY.shape[ , , outliers[2]], method = "vector")

```

#Basic allometry analysis
```{r}
#Is there allometry in the whole sample?

shape.size <- geomorph.data.frame(shape = bY.shape, size = sp.info$Csize)
