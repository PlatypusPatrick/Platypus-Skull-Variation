...
### Is the allometry correlated with sex? (Part 1)
```{r}
...
#Plot
plot(Allom_Sex, type = "regression", reg.type = "RegScore", predictor =AllometryGDF$size, pch = 19, col= grp.col)
plot(Allom_Sex, type = "regression", reg.type = "PredLine", predictor =AllometryGDF$size, pch = 19, col= grp.col)
legend("topright")
...
### Is the allometry correlated with individual sex? (Part 2)
```{r}
...
Allom_Lat.lm <-procD.lm(shape~size+Lat, iter = 499, data = AllometryGDF)
summary(Allom_Lat.lm)
...
### Is the allometry correlated with population? (Part 1)
```{r}
...
### Is the allometry correlated with individual populations? (Part 2)
```{r}
#Allometry with individual populations - Mainland
index.main <- which(meta.info$Population == "Mainland")

#VW TRIAL for combining Tassie and
...
#No significand interactions means that slopes are not different
...
AllometryGDF.tas <- geomorph.data.frame(shape=shape.tas, size=metadata.tas$Csize, pop= metadata.tas$Population)

Allom_Pop_Tas <-procD.lm(shape~size, iter = 499, data = AllometryGDF.tas)
