...
Viewbox puts the most recent specimen in the first row of the output .csv file, which is why we take the 10 initial specimens from the last rows of the coordinate dataset. 

##VW: DO YOU NEED THE BELOW? IT SHOULD WORK AS IS BECAUSE THIS IS RMD. I AM RUNNING EVERYTHING FINE WITHOUT IT

```{r}
setwd("D:/Github_folders/Platypus-Skull-Variation/1_Data (original data)/1_Raw") 
```
...
### Convert to 3D array
Here, we convert the coordinates into a 3D array for _geomorph_. Importantly, we need to remove the 1st column of landmark names. Followig this is a 3D check of the landmarked positions. 
```{r}
...
Interspersed throughout the data collection process, PG landmarked 10 specimens twice. Their specimen names included "_ERR", however, not always in the same place in the filename. 
...
```{r}
#brief check that metadata and Csize match - this should return "TRUE" for all
...
```{r}
#again, flexible coding the mean specimen into a number so adding more data that might change the mean specimen isn't a problem
meanspec=unname(findMeanSpec(bY.shape))

par <- gridPar(pt.bg="pink", pt.size = 1)

#make sure that you compare outliers against mean specimens and nothing else at all times
plotRefToTarget(bY.shape[ , , meanspec], bY.shape[ , , 56], method = "vector", gridPars = par )
plotRefToTarget(bY.shape[ , , outliers[2]], bY.shape[ , , outliers[29]], method = "vector")

```

#Align the specimen metadata with the coordinates for allometry analyses
```{r}
#first ensuring that sp.infor and bY.shape have the specimens in the same order, which they do not by default because bilat.symm changes the order
#First, take out the "ind" that is left over from the bilat symm operation; this makes the dimnames comparable to sp.info$All
dimnames(bY.shape)[[3]] <- sub(".*ind", "", dimnames(bY.shape)[[3]])

#if the below gives a bunch of numbers but no NA, we are OK to re-order
...
Allom <-procD.allometry(shape~size, iter = 499, data = shape.size)

summary(Allom)

plot(Allom, method="RegScore" )
#if labels are needed for the allometry plot
label=dimnames(shape.size$shape)[[3]] #put inside the brackets ^ - ,label=....
```
...
#VW: The below should not give NAs if the names of one data frame are present in the other
...
#VW: Making sure bY.shape and meta.info$Csize match up; they don't seem to!
dimnames(bY.shape)[[3]]
rownames(meta.info)


#Allometry investigating different factors
AllometryGDF <- geomorph.data.frame(shape=bY.shape, size=meta.info$Csize, sex= meta.info$Sex, Lat=meta.info$Latitude)

#VW: Check if using this data frame returns the same allometry result if run without any other factors - a good way of checking if all is still well!
Allom_simple <-procD.allometry(shape~size, iter = 499, data = AllometryGDF)
summary(Allom_simple)

Allom_Sex <-procD.allometry(shape~size, ~sex, iter = 499, data = AllometryGDF)
summary(Allom_Sex)

Allom_Lat <-procD.lm(shape~size*Lat, iter = 499, data = AllometryGDF)
summary(Allom_Lat)

plot(Allom_Lat,method="RegScore")

example:
(x, type = c("diagnostics", "regression", "PC"),  outliers = FALSE, predictor = NULL, reg.type = c("CRC", "PredLine", "RegScore"), ...)
https://www.rdocumentation.org/packages/geomorph/versions/3.0.7/topics/plot.procD.lm

plot(Allom_Lat, type = "regression"), predictor = meta.info$Latitude)
...
Note - hard to say what "pop" they are part of without defining that first.
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

#correlate csize with sex, population and latitude without shape
       #check out procD.pgls in the geomorph help files
CS_Sex <- procD.pgls(Shape ~CS, iter = 499, data = AllometryGDF) #example code from Ariel's work
Summary(CS_Sex)
