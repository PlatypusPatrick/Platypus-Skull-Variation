...
LM_SD <- procD.lm(size~sex, iter = 499, data = AllometryGDF_MF)
summary(LM_SD) #Yes! And a huge difference, too - 49% of size explained by sex.

plot(LM_SD, type = "regression", reg.type = "RegScore", predictor = AllometryGDF_MF$size, pch = 19)
```

### Test for Bergmann's Rule and sexual dimorphims (i.e. is centroid size influenced by latitude?)
```{r}
#Linear Model - Does size change with Lat?
LM_BR <- procD.lm(size~lat, iter = 499, data = AllometryGDF)
summary(LM_BR)

plot(LM_BR, type = "regression", reg.type = "RegScore", predictor =AllometryGDF$lat, pch = 19)
```
...
### Individual sub-populations (Part 2)
```{r}
...
### Individual sub-populations (Part 3) - Mainland vs Tasmania
```{r}
...
### Individual sub-populations (Part 4) - Mainland vs West Victoria
```{r}
