...
plot(Allom, method="RegScore" ); labels=dimnames(shape.size$shape)[[3]] 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  Error to fix - Plots above do not work anymore
...
##We will start with defining our data frame and looking at allometry plotted
```{r}
#Define the data frame
AllometryGDF <- geomorph.data.frame(shape=bY.shape, size=meta.info$Csize, sex= meta.info$Sex, Lat=meta.info$Latitude)

#Allometry (same as above but checking with added AllometryGDF - Gives the same results)
Allom_simple <-procD.lm(shape~size, iter = 499, data = AllometryGDF)
summary(Allom_simple)

#############Some test snippets for figuring out the new allometry plotting

Allom_simple_log <-procD.lm(shape~log(size), iter = 499, data = AllometryGDF)
summary(Allom_simple)
summary(Allom_simple_log)

labels=as.vector(dimnames(AllometryGDF$shape)[[3]])
...
##Is the allometry correlated with sex?
```{r}
## SEX
...
plot(meta.info$Csize, Allom_Sex$Reg.proj, xlab = "Centroid size", ylab= "Regression Score", cex=0.5, pch = 16, col= grp.col)
...
Allom_Sex.lm <-procD.lm(shape~size+sex, iter = 499, data = AllometryGDF)
summary(Allom_Sex.lm) 
...
AllometryGDF.m <- geomorph.data.frame(shape=shape.m, size=metadata.m$Csize, sex= metadata.m$Sex)

Allom_Sex_M <-procD.lm(shape~size, iter = 499, data = AllometryGDF.m)
summary(Allom_Sex_M)
...
AllometryGDF.f <- geomorph.data.frame(shape=shape.f, size=metadata.f$Csize, sex= metadata.f$Sex)

Allom_Sex_F <-procD.lm(shape~size, iter = 499, data = AllometryGDF.f)
summary(Allom_Sex_F) 

#Weight and Centroid size - very signifcant 
...
##Is the allometry correlated with latitude?
```{r}
...
#Latitude and Centroid size - not signifcant 
summary(lm(meta.info$Csize~meta.info$Latitude))
```

##Is the allometry correlated with population?
```{r}
#Allometry looking at Population difference
AllometryGDF.pop <- geomorph.data.frame(shape=bY.shape, size=meta.info$Csize, pop= meta.info$Population)

#Allometry (linear model) with no interaction 
Allom_Pop <-procD.lm(shape~size+pop, iter = 499, data = AllometryGDF.pop)
summary(Allom_Pop)

#Allometry (linear model) with interaction 
Allom_Pop.lm <-procD.lm(shape~size*pop, iter = 499, data = AllometryGDF.pop)
summary(Allom_Pop.lm)

#Allometry with individual populations as an interaction - Mainland
index.main <- which(meta.info$Population == "Mainland")
metadata.main <- meta.info[index.main, ]
shape.main <- bY.shape[, , index.main]
AllometryGDF.main <- geomorph.data.frame(shape=shape.main, size=metadata.main$Csize, pop= metadata.main$Population)

Allom_Pop_Main <-procD.lm(shape~size*pop, iter = 499, data = AllometryGDF.main)
