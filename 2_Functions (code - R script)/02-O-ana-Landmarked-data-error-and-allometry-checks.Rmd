...
plot(AllometryGDF_MF$size~AllometryGDF_MF$sex, pch = 19)
```
...
plot(AllometryGDF$size~AllometryGDF$lat, pch = 19)
```
...
metadata_MFP <- meta.info[-c(toberemoved_MF,toberemoved_NQ), ]
shape_MFP <- bY.shape[, , -c(toberemoved_MF,toberemoved_NQ)]
AllometryGDF_PopSex <- geomorph.data.frame(shape=shape_MFP, size=metadata_MFP$Csize, sex= metadata_MFP$Sex, pop=metadata_MFP$Population)


#Size <- geomorph.data.frame(shape=bY.shape, size=meta.info$Csize, pop= meta.info$Population, sex=meta.info$Sex)


#Linear Model - Does size change with populations  
LM_POP <- lm(AllometryGDF_PopSex$size~AllometryGDF_PopSex$pop)
summary(LM_POP)

plot(AllometryGDF_PopSex$size~AllometryGDF_PopSex$pop, pch = 19)

#Testing if the lack of differences between populations is due to the strong size dimorphism
LM_POP2 <- lm(AllometryGDF_PopSex$size~AllometryGDF_PopSex$pop*AllometryGDF_PopSex$sex)
summary(LM_POP2) #Very low significance interaction between male and Tassie, omitting due to very small effect size

#Dropping interaction - there is no significant difference is size betwen populations, even after acocunting for size
LM_POP2 <- lm(AllometryGDF_PopSex$size~AllometryGDF_PopSex$sex+AllometryGDF_PopSex$pop)
summary(LM_POP2) #
...
## Is there allometry correlated (shape as a function of size) with sex; latitude; population?
