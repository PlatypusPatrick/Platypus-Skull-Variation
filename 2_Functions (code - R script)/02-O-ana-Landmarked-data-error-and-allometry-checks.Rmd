...
AllometryGDF_Main <- geomorph.data.frame(shape=shape.main, size=metadata.main$Csize, pop= metadata.main$Population) #Define a new general data frame
...
AllometryGDF_Tas <- geomorph.data.frame(shape=shape.tas, size=metadata.tas$Csize, pop= metadata.tas$Population) #Define a new general data frame

Allom_Pop_Tas <-procD.lm(shape~size, iter = 499, data = AllometryGDF_Tas)
summary(Allom_Pop_Tas)  
```

### Is the allometry correlated with individual populations? - Mainland vs Tasmania (Part 3)
```{r}
...
### Is the allometry correlated with individual populations? - Mainland vs West Victoria (Part 4)
```{r}
...
### Test for Bergmann's Rule
```{r}
#Linear Model - Does size change with Lat?
LM_BR <- procD.lm(size~lat, iter = 499, data = AllometryGDF)
summary(LM_BR)

#Linear Model - Does shape change with Lat?
LM_BR_1 <- procD.lm(shape~lat, iter = 499, data = AllometryGDF)
summary(LM_BR_1)


### !!!! This one is the correct test to see test Bergmann's Rule I belive !!!!
#Linear Model - Does size change with Lat?  !!!!
LM_BR_2 <- procD.lm(lat~size, iter = 499, data = AllometryGDF)
summary(LM_BR_2)

plot(LM_BR_2, type = "regression", reg.type = "PredLine", predictor =AllometryGDF$lat, pch = 19)
```
...
### PCA plotting 
```{r}
pca.general <- plotTangentSpace(bY.shape)

#Make a colour key
colour.key2 <- c("Red","Blue", "Black", "Green", "Purple")
grp <- as.factor(meta.info$Sex)
names(colour.key2) <- sort(unique(grp))
grp.col_2 <- colour.key2[match(grp, names(colour.key2))]

library(plotrix) #don't know if I need

pca_sex <- PlotByGroup(meta.info, "Sex", grp.col_2)
summary(pca_sex) #?
PCA_Sex <- plotTangentSpace(pca_sex) #?
plot(pca_sex) #?

PCA_Lat <- PlotByGroup(meta.info, "Latitude", grp.col_2)
