...
plot(AllometryGDF_MF$size~AllometryGDF_MF$sex, pch = 19, xlab = "Sex", 
                         ylab = "Centroid Size",)
...
#plot
plot(metadata_MF$Csize~metadata_MF$Weight..g.,col=factor(metadata_MF$Sex), xlab = "Weight (g)", 
                         ylab = "Centroid Size")
...
LM_POP2 <- lm(LM_GDF_PopSex$size~LM_GDF_PopSex$pop*LM_GDF_PopSex$sex)
...
Allom_Sex_MF <-procD.lm(shape~sex*size, iter = 499, data = AllometryGDF_MF)
summary(Allom_Sex_MF) 

#Taking out the interaction
Allom_Sex_MF_NoInter <-procD.lm(shape~sex+size, iter = 499, data = AllometryGDF_MF)
...
plot(Allom_Sex_MF_NoInter, type = "regression", reg.type = "RegScore", predictor = AllometryGDF_MF$s, pch = 19, col= grp.col)
legend("bottomright", levels(AllometryGDF_MF$Sex), col=grp.col, pch = 19)
```

### Latitude (shape as a function of latitude)
```{r}
#Linear Model - Does shape change with size and latitude
Allom_Lat <-procD.lm(shape~lat, iter = 499, data = AllometryGDF)
summary(Allom_Lat)
```

### Populations (shape as a function of Population) (Part 1)
```{r}
#Define a new general data frame
...
Allom_Pop <-procD.lm(shape~pop*size, iter = 499, data = AllometryGDF_Pop)
summary(Allom_Pop)

#No interaction 
Allom_Pop_NoInter <-procD.lm(shape~pop, iter = 499, data = AllometryGDF_Pop)
summary(Allom_Pop_NoInter)
```

### Individual populations (shape as a function of Population) (Part 2) - Mainland vs Tasmania
```{r}
...
#Linear Model - Does shape change population (Mainland vs Tasmanian) with size
Allom_Pop_MT <-procD.lm(shape~pop*size, iter = 499, data = AllometryGDF_MT)
summary(Allom_Pop_MT)
#No significant interaction means that slopes are not different

#No interaction 
Allom_Pop_MT_NoInter <-procD.lm(shape~pop+size, iter = 499, data = AllometryGDF_MT)
summary(Allom_Pop_MT_NoInter)
```

### Individual populations (shape as a function of Population) (Part 3) - Mainland vs West Victoria
```{r}
...
#Linear Model - Does shape change population (Mainland vs West Victoria) with size
Allom_Pop_MWV <-procD.lm(shape~pop*size, iter = 499, data = AllometryGDF_MWV)
summary(Allom_Pop_MWV)
#No significant interaction means that slopes are not different

#No interaction 
Allom_Pop_MWV_NoInter <-procD.lm(shape~pop+size, iter = 499, data = AllometryGDF_MWV)
...
colour_pca_sex <- c("Black", "Red")
grp <- as.factor(metadata_MF$Sex)
names(colour_pca_sex) <- sort(unique(grp))
grp_col_pca_sex <- colour_pca_sex[match(grp, names(colour_pca_sex))]

# Plot 3D shape into a PCA plot (excluding the NA's to sex)
pca_general_MF <- plotTangentSpace(shape_MF)

#Summary of PCA
pca_general_MF$pc.summary

#Look at difference
plotRefToTarget(pca_general_MF$pc.shapes$PC1min,pca_general_MF$pc.shapes$PC1max, method="vector",pt.bg="pink")

#Plot PCA of Sex
plot(x = pca_general_MF$pc.scores[, 1],
                         y = pca_general_MF$pc.scores[, 2], 
                         xlab = "PC1", 
                         ylab = "PC2",
                         asp = TRUE,
                         col = grp_col_pca_sex, 
                         pch = 16 )
legend("topleft", levels(metadata_MF$Sex), col=colour_pca_sex, pch = 19)
