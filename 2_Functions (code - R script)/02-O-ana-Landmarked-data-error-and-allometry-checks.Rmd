...
```{r}
#Define a new general data frame
AllometryGDF_Pop <- geomorph.data.frame(shape=bY.shape, size=meta.info$Csize, pop= meta.info$Population, sex=meta.info$Sex)

#Linear Model - Does size change with populations  
LM_POP <- lm(AllometryGDF_Pop$size~AllometryGDF_Pop$pop)
summary(LM_POP)

plot(AllometryGDF_Pop$size~AllometryGDF_Pop$pop, pch = 19)

#Trial (size)~pop+sex
LM_POP2 <- lm(AllometryGDF_Pop$size~AllometryGDF_Pop$pop+AllometryGDF_Pop$sex)
summary(LM_POP2)

#trial
LMD_POP <-procD.lm(size~pop, iter = 499, data = AllometryGDF_Pop)
summary(Allom_Pop)
...
#trial
LMD_POP2 <-procD.lm(size~pop*sex, iter = 499, data = AllometryGDF_Pop)
summary(LMD_POP2)
