---
title: "01-O-ana-extract-data-for-analyses"
...
```{r}
setwd ("P:/Documents/UQ/1 - Honours/1 - BIOL6503 - Research Project/1) Research Project/3) Data (original data)/1 - Raw")

coords.3D <- read.csv("X01_O_ana_-_Repeatability_Error_Check", header = T)
```
...
```{r}
data.raw <- as.matrix(X01_O_ana_Repeatability_Error_Check)
data <- X01_O_ana_Repeatability_Error_Check[, 2:(dim(X01_O_ana_Repeatability_Error_Check)[2])]  # removes LM names for geomorph
data <- t(data)
...
Now that we have extracted the dataset and the specimen metadata, we are ready for the next analysis step: 02-calculate-user-error.Rmd.
